...
def play(cfg, dataset, name):
    ...
    video = cv2.VideoWriter(cfg['outdir'] + 'play/%s.mp4' % (name), fourcc, 10, (810, 1080))
    for i, (imgs, target) in enumerate(dataset):
        ...
        cv2.imwrite(cfg['outdir'] + 'play/render/%s/%06d.jpg' % (name, i), (img * 255).astype(np.uint8))
        # cv2.waitKey(1)
    print('Saving video...')
    video.release()
    print('Video saved.')
