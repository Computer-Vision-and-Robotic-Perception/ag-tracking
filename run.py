...
if __name__ == '__main__':
    ...
    if 'lettuce' in args.dataset.lower(): 
        cfg = cfg_lettuce
        print("Collection:", "LettuceMOT")
    if 'apple' in args.dataset.lower(): 
        cfg = cfg_apples
        cfg['datadir'] += 'MOT/'
        cfg['outdir'] += 'MOT/'
        print("Collection:", "AppleMOTS")
    ...
    if cfg['task'] == 'track':
        for dataloader, name in zip(dataloaders, cfg['sets']):
            print('\n' * 2 + 'dataset:', name)
            os.makedirs(cfg['outdir'] + 'track', exist_ok=True)
